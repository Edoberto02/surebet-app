...
function StatusPills({
  ...
  const openActive = isDay ? "bg-blue-200 text-blue-900 border-blue-300" : "bg-blue-700/70 text-blue-100 border-blue-600";
  const winActive = isDay ? "bg-emerald-200 text-emerald-900 border-emerald-300" : "bg-emerald-700/80 text-emerald-100 border-emerald-600";
  const lossActive = isDay ? "bg-red-200 text-red-900 border-red-300" : "bg-red-800/70 text-red-100 border-red-700";
...
export default function ScommessePage() {
  ...
  // ✅ BLOCCO ANTI MULTI-CLICK (questa è la cosa che ti mancava davvero)
  ...
  async function loadAll() {
    ...
    const { data: posAcc, error: posErr } = await supabase.from("accounts").select("id,person_name,bookmaker_name,balance").gt("balance", 0);
    if (posErr) {
    ...
    if (idsFromLegs.length > 0) {
      const { data: used, error: usedErr } = await supabase.from("accounts").select("id,person_name,bookmaker_name,balance").in("id", idsFromLegs);
      if (usedErr) {
  ...
  const accountMeta = useMemo(() => {
    ...
    for (const a of accounts) {
      m.set(a.id, { label: `${a.bookmaker_name} — ${a.person_name}`, slug: slugifyBookmaker(a.bookmaker_name) });
    }
    return m;
  }, [accounts]);
  ...
  const closedGrouped = useMemo(() => {
    const monthMap = new Map<
      string,
      { monthProfit: number; days: Map<string, { dayProfit: number; bets: BetSummary[] }> }
    >();
  ...
  // ✅ QUESTA È LA FUNZIONE FIXATA: 1) blocca multi-click, 2) rimette saving=false SEMPRE
  async function saveNewBet() {
    if (saving) return; // ✅ se sta già salvando, ignora click successivi
    ...
    return (
      <div className={`${innerCls} relative px-3 py-2 pt-3 text-xs`}>
        ...
        <div className="flex items-center justify-between gap-2">
          <div className={isDay ? "text-slate-600 font-semibold" : "text-zinc-500 font-semibold"}>Leg {idx + 1}</div>
        </div>

        <div className={isDay ? "mt-1 text-slate-700" : "mt-1 text-zinc-300"}>{meta?.label ?? leg.account_id}</div>
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          ...
          <button
            ...
            className={["rounded-lg px-3 py-1 text-xs font-semibold transition", "bg-[#163D9C] text-white hover:bg-[#12337F]"].join(" ")}
          >
            Modifica
          </button>
        </div>
  ...
  return (
    <main className={`${pageCls} p-6`}>
      ...
      {msg && <div className={`mt-4 whitespace-pre-wrap rounded-xl p-3 text-sm ${innerCls}`}>{msg}</div>}
          ...
          <div className="mt-6 overflow-hidden rounded-2xl border border-blue-200">
                ...
                <label className="text-sm text-blue-100">
                  ...
                  <select
                    ...
                    className={[
                      ...
                      isDay ? "border border-blue-200 bg-white/95 text-slate-900" : "border border-zinc-700 bg-zinc-950 text-white",
                    ].join(" ")}
            ...
            <div className={`${panelCls} p-4`}>
              ...
              <div className={`mt-4 ${innerCls} p-3`}>
                <div className={`text-sm font-semibold ${isDay ? "text-slate-800" : "text-zinc-200"}`}>Chi ha giocato la bet (bonus 10%)</div>
                ...
                {newLegs.map((l, i) => (
                  <div key={i} className={`${innerCls} p-3`}>
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-5">
                      ...
                      <label className={`text-sm ${isDay ? "text-slate-700" : "text-zinc-300"}`}>
                        Importo
                        <input value={l.stake} onChange={(e) => updateNewLeg(i, { stake: e.target.value })} className={inputCls} />
                      </label>

                      <label className={`text-sm ${isDay ? "text-slate-700" : "text-zinc-300"}`}>
                        Quota
                        <input value={l.odds} onChange={(e) => updateNewLeg(i, { odds: e.target.value })} className={inputCls} />
                      </label>
              ...
              {/* ✅ BOTTONE FIXATO: disabled + testo "Salvataggio…" */}
              <button
                type="button"
                disabled={saving}
                onClick={saveNewBet}
                className={[
                  "mt-6 rounded-xl px-4 py-2 text-sm font-semibold text-white transition",
                  saving ? "bg-blue-500 opacity-60 cursor-not-allowed" : "bg-blue-800 hover:bg-blue-700",
                ].join(" ")}
              >
                {saving ? "Salvataggio…" : "Salva bet"}
              </button>
            </div>
          </div>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className={[
                            ...
                            isDay ? "bg-white text-[#163D9C] border border-[#163D9C] hover:bg-blue-50" : "bg-zinc-800 text-zinc-200 hover:bg-zinc-700",
                          ].join(" ")}
                        >
                          Modifica data/ora
                        </button>
                        ...
                        <button
                          ...
                          className={["rounded-xl px-3 py-2 text-xs font-semibold transition text-white", "bg-red-600 hover:bg-red-500"].join(" ")}
                        >
                          Elimina
                        </button>
                      </div>
            ...
            <div className="bg-gradient-to-r from-[#163D9C] to-blue-600 px-6 py-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1 text-sm text-blue-100">Archivio delle bet concluse (raggruppate per mese e giorno)</div>
                </div>

                <div
                  className={[
                    "rounded-xl border px-3 py-2 text-sm font-semibold",
                    isDay ? "border-blue-200 bg-white/95 text-slate-900" : "border-zinc-700 bg-zinc-900 text-zinc-100",
                  ].join(" ")}
                >
                  {closed.length} bet
                </div>
                  ...
                  {closedGrouped.map((m) => (
                    <details key={m.monthStart} className={`${innerCls}`}>
                      <summary className="cursor-pointer select-none px-4 py-3 flex items-center justify-between">
                        <div className={`text-sm font-semibold ${isDay ? "text-slate-900" : "text-zinc-100"}`}>{monthLabel(m.monthStart)}</div>
                      ...
                      <div className="px-4 pb-4 space-y-2">
                        {m.days.map((d) => (
                          <details key={d.dayISO} className={`${innerCls}`}>
                            <summary className="cursor-pointer select-none px-4 py-3 flex items-center justify-between">
                              <div className={`text-sm ${isDay ? "text-slate-900" : "text-zinc-100"}`}>{formatDateIT(d.dayISO)}</div>
                                    ...
                                    <div className="text-xs text-right space-y-1">
                                      {Array.from(splitProfitWithBonus(bs.profit, bs.bet.id).entries()).map(([name, val]) => (
                                        <div key={name} className="font-semibold">
                                          <span className={isDay ? "text-slate-900" : "text-zinc-100"}>{name}:</span>{" "}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 p-4 flex items-center justify-center">
          ...
          >
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
              <label className={`text-sm ${isDay ? "text-slate-700" : "text-zinc-300"}`}>
                Data
                <input type="date" value={editBetDate} onChange={(e) => setEditBetDate(e.target.value)} className={inputCls} />
              </label>

              <label className={`text-sm ${isDay ? "text-slate-700" : "text-zinc-300"}`}>
                Ora
                <input type="time" value={editBetTime} onChange={(e) => setEditBetTime(e.target.value)} className={inputCls} />
              </label>
            </div>

            <button onClick={saveEditBet} className={["mt-6 rounded-xl px-4 py-2 text-sm font-semibold transition text-white", "bg-[#163D9C] hover:bg-[#12337F]"].join(" ")}>
              Salva
            </button>
          </div>
        </div>
      ...
      {openEdit && (
        <div className="fixed inset-0 z-50 bg-black/60 p-4 flex items-center justify-center">
          ...
          >
            ...
            {editErr && <div className="mt-4 rounded-xl border border-red-900 bg-red-950/40 px-4 py-3 text-sm text-red-200">{editErr}</div>}

            <div className="mt-4 grid grid-cols-1 gap-3">
              <div>
                <SearchSelect label="Bookmaker / Persona" value={editAccountId} options={allAccountOptions} onChange={setEditAccountId} isDay={isDay} />
              </div>

              <label className={`text-sm ${isDay ? "text-slate-700" : "text-zinc-300"}`}>
                Importo
                <input value={editStake} onChange={(e) => setEditStake(e.target.value)} className={inputCls} />
              </label>

              <label className={`text-sm ${isDay ? "text-slate-700" : "text-zinc-300"}`}>
                Quota
                <input value={editOdds} onChange={(e) => setEditOdds(e.target.value)} className={inputCls} />
              </label>

              <button onClick={saveEditLeg} className="rounded-xl bg-blue-800 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 transition">
                Salva modifiche
              </button>
            </div>
          </div>
